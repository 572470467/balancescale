...
class Balance(object):
    ...
    def Test():#静平衡仪测试界面部分
        ...
        pygame.draw.rect(screen,Green,[100,735,110,35],0)
        text_fmt=text_1.render("开始测试",2,Black)
        screen.blit(text_fmt,(116,739))
        pygame.draw.rect(screen,Green,[260,735,110,35],0)
        text_fmt=text_1.render("返回主界面",2,Black)
        screen.blit(text_fmt,(266,739))
    ...
    def flip_over(page,lt):#翻页
        for i in range(len(lt)):
            ...
            if int((page-1)*19)<=i<=int(page*19-1) and 1<page<=n and n!=1:
                ...
                ls=[["{}".format(lt[i][0][0:10]),55],["{}".format(lt[i][11]),175],["{}".format(lt[i][1]),285],["{}".format(lt[i][3]),375]]
                for t in ls:
    ...
    def Today(page):#当日数据
        ...
        if start==3:
            ...
            for a in reader:
                if a[0][0:10]==day:
                    if int(a[2]) > dg or Wb + dW < int(a[12]) < Wb - dW:
                        lt.append(a)            
                    ...
                    if pressed_array[index]:
                        if index==0:
                            if 140<=pos[0]<=225 and 350<=pos[1]<=380 and start==0:#默认测试按钮
                                ...
                                start=0
                                Balance.Main()
                            elif 260<=pos[0]<=370 and 735<=pos[1]<=760 and start==1:#测试界面返回按钮
                                start=0
                                Balance.Main()                                
                            elif 100<=pos[0]<=210 and 735<=pos[1]<=760 and start==1:#开始测试按钮
                                J+=1
                                response=urllib.request.urlopen("http://localhost:5000/balance/measure/")
                                html=response.read().decode()
                                text=json.loads(html)
                                list=[[text['weight'],395],[text['balance'],455],[text['angle'],515],[J,575]]
                                pygame.draw.circle(screen,White,(240,200),118,0)
                                for c in list2:
                                    pygame.draw.aaline(screen,Black,(c[0],c[1]),(c[2],c[3]))
                                for i in list0:
                                ...
                                pygame.draw.rect(screen,Black,[365,470,80,250],0)
                                ...
                                pygame.display.update()
                            elif 260<=pos[0]<=370 and 735<=pos[1]<=70 and start==1:#测试界面返回按钮
                                start=0
                                Balance.Main()
